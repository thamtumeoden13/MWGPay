...
export const DeliveringDetailExpand = (props) => {
    ...
    useEffect(() => {
        ...
        let isMaterialList = false
        if (CurrentStepPermissionList && CurrentStepPermissionList.filter(e => { return e.ShipmentOrderPermissionID == 14 }).length) {
            isMaterialList = true
        }
        if (CurrentStepPermissionList && CurrentStepPermissionList.filter(e => { return e.ShipmentOrderPermissionID == 21 }).length) {
            isTakePhoto = true
        }
        setPermission({ ...permission, takePhoto: isTakePhoto, materialList: isMaterialList })

    }, [CurrentStepPermissionList])
            ...
            <KeyboardAwareScrollView contentContainerStyle={{
                ...
                <View style={styles.containerDetail}>
                    ...
                    <View style={styles.delivery}>
                        <View style={styles.deliveryChild}>
                        ...
                        <View style={styles.deliveryChild}>
                            <Icon name='directions' type='material' color='#000' size={24} />
                            <Text style={{ fontSize: 16, fontWeight: '500', color: '#000', paddingLeft: 5, paddingRight: 10 }}>{props.estimateDeliveryDistance.text}</Text>
                        </View>
                    </View>
                ...
                {(CombinedMaterialList && CombinedMaterialList.length > 0) ?
                    <View style={styles.containerData}>
                        ...
                        {!permission.materialList &&
                            <Text style={{ fontSize: 16, color: 'red', left: 10 }}>Không có quyền thay đổi số lượng vật tư</Text>
                        }
                        {permission.materialList &&
                                    ...
                                    <View style={{ flexDirection: 'row', alignItem: 'center', justifyContent: 'space-between', margin: 5 }}>
                                        <View style={{ flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                                            ...
                                            <Input
                                                ...
                                                disabled={!permission.materialList}
                                            />
                                        </View>
                                        <View style={{ flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                            <ButtonBottomComponent
                                title="Cập nhật"
                                onPress={() => props.updateSaleMaterial(materialList)}
                                containerButtonGroup={styles.buttonMaterialList}
                                disabled={!permission.materialList}
                            />
