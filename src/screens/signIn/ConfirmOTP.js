...
class ConfirmOTPCom extends Component {
    ...
    componentDidMount() {
        ...
        const uniqueId = this.props.navigation.getParam('uniqueId', '');
        const modelName = this.props.navigation.getParam('modelName', '');
        this.setState({
            walletRegOTPId,
            phoneNumber,
            password,
    ...
    reSendOTP = (phoneNumber) => {
        ...
        const SearchAPIPath = "api/Wallet/ResentDeviceRegisterOTP";
    ...
    onConfirmOTP = (OTP) => {
        let { countCallOTP, phoneNumber, walletRegOTPId, password, uniqueId, modelName } = this.state
        this.setState({
            countCallOTP: countCallOTP + 1
        })
        if (OTP && OTP.length > 0 && countCallOTP < 5) {
            ...
            const signature = createSignature(walletRegOTPId + OTP + phoneNumber + password + uniqueId + modelName);
            const walletRegOTP = {
    ...
    verifyOTP = (walletRegOTP) => {
        let { countCallOTP } = this.state
        const APIHostName = "CustomerEWalletAPI";
        const SearchAPIPath = "api/Wallet/RegisterDevice";
        this.props.callAPIWithoutAuthen(APIHostName, SearchAPIPath, walletRegOTP).then(apiResult => {
            if (apiResult.IsError) {
                ...
                setTimeout(() => {
                    ...
                    this.props.navigation.state.params.onPreSignIn();
                }, 500);
            }
        });
    }
