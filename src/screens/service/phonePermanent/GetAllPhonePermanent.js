...
import { Service_Category_ID_PhonePermanent } from '@constants/appSetting';
import { CDN_IMG } from "@constants/systemVars.js";

class GetAllPhonePermanentCom extends Component {
    ...
    getCacheServiceByCategoryID = (result) => {
        ...
        let resultPhonePostPaid = result.filter((item, index) => {
            return item.ServiceCategoryID == Service_Category_ID_PhonePermanent && item.IsActived == true
        });
...
const GetAllPhonePermanent = connect(mapStateToProps, mapDispatchToProps)(GetAllPhonePermanentCom);
export default GetAllPhonePermanent;
